...
# 💡 현재 PC 경로로 수정
base_dir = r'C:\Users\PC\OneDrive\바탕화면'

# -----------------------------------------------------------------------------
# 2. 데이터 로드 함수 (경로 탐색 단순화)
# -----------------------------------------------------------------------------
@st.cache_data
...
# -----------------------------------------------------------------------------
# 3. 사이드바 비교 필터 (단일 일자 전용 체크박스 옵션 추가)
# -----------------------------------------------------------------------------
...
if raw_df is None:
    st.error(f"❌ {target_year}년 시청률 분석 데이터 CSV 파일을 찾을 수 없습니다.")
    st.stop()
...
# 3) 날짜 필터 설정 (💡 단일 일자 전용 체크박스 옵션 추가)
st.sidebar.markdown("---")
...
# -----------------------------------------------------------------------------
# 4. 상단 KPI 카드 (단일 일자 vs 기간 평균 동적 표출)
# -----------------------------------------------------------------------------
if is_single_day:
    st.subheader(f"📅 [{start_date.strftime('%Y-%m-%d')}] 당일 시청률 성과 요약")
else:
...
# ... (탭 1 ~ 탭 3 그래프 코드는 동일 유지) ...

# -----------------------------------------------------------------------------
# 📌 탭 4: 정밀 통계 표 (날짜 포맷 정제 적용)
# -----------------------------------------------------------------------------
with tab4:
    st.subheader("📋 선택 정밀 시청률 데이터 표")
    
    # 💡 원본 filtered_df는 건드리지 않고, 표시용 사본(display_df)을 만들어 방송일자를 YYYY-MM-DD 문자열로 변환
    display_df = filtered_df.copy()
    display_df['Date_Only'] = display_df['Date'].dt.strftime('%Y-%m-%d')
    
    # 대시보드 표에 보여주기
    st.dataframe(
        display_df[['Date_Only', 'Program_Name', 'DayName', 'Rating', 'WoW_Diff', 'Z_Score']].rename(
            columns={'Date_Only':'방송일자', 'Program_Name':'프로그램', 'DayName':'요일', 'Rating':'시청률(%)', 'WoW_Diff':'전주대비(%p)'}
        ),
        use_container_width=True
    )
    
    # CSV 다운로드 파일 생성 시에도 날짜 포맷 적용
